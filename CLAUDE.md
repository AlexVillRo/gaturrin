...
- **API REST**: `status`, `visits` (+`/reassign`), `sync`, `resync`, `clean`, `cancel`, `cmd/:code/:value`, `spec`, `info`, `litterboxes` (+`/discover`, `/save`, `/delete`, `/assign`), `cats` (+`/save`, `/delete`), `avatars`, `maintenance` (+`/reset`, `/threshold`), `schedules` (+`/save`, `/delete` — jobs pg_cron), `notifyprefs` (+`/save`), `telegram` (webhook de botones), `alexa` (skill propio, firma de Amazon en vez de PIN; modelo en `docs/alexa-skill.json`), `drawer/reset` (lo llama la página `/limpie?token=<DRAWER_TOKEN>` de la etiqueta NFC/QR — el reinicio va por POST desde JS para que las vistas previas de enlaces no lo disparen), `login`, `tuyapage`/`rawlogs`/`logscan` (debug). Multi-arenero vía query `?device=<id>`.
