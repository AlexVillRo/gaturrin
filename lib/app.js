...
async function alreadyNotified(key, now) {
  if (_lastNotified[key] && now - _lastNotified[key] < ALERT_COOLDOWN) return true;
  if (db) {
    try {
      const { rows } = await db.query('SELECT ts FROM alerts_sent WHERE key=$1', [key]);
      if (rows.length && now - Number(rows[0].ts) < ALERT_COOLDOWN) return true;
        ...
        try {
          ...
          if (pct >= 100) {
            await notifyOnce('drawer100:' + lb.device_id,
              ...
              ' visitas desde el último vaciado). Vacíalo y toca "Ya lo vacié" en la app.');
          } else if (pct >= 80) {
            await notifyOnce('drawer80:' + lb.device_id,
              '🗑️ El cajón de <b>' + nombre + '</b> va ~' + pct + '%. Conviene vaciarlo pronto.');
          }
