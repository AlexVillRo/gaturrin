...
const handler = async function(req, res) {
  ...
  if (pathname.startsWith('/api')) {
    ...
    try {
      ...
      // TEMP: prueba de envío a todos los chat_id configurados (quitar después).
      if (pathname === '/api/telegram/test' && req.method === 'GET') {
        const ok = await sendTelegram('🐱 Mensaje de prueba de Gaturrin — si ves esto, las notificaciones te están llegando bien.');
        json({ success: ok, chat_ids: TG_CHAT_IDS });
        return;
      }
