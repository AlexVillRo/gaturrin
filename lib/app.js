...
// chat_id escribiéndole al bot y consultando /getUpdates. TELEGRAM_CHAT_ID
// admite varios ids separados por coma (ej. para compartir las alertas con
// la pareja) — cada uno recibe los mismos mensajes y puede usar los botones.
const TG_TOKEN      = process.env.TELEGRAM_BOT_TOKEN || null;
const TG_CHAT_IDS   = String(process.env.TELEGRAM_CHAT_ID || '')
  .split(',').map(s => s.trim()).filter(Boolean);
// Etiqueta NFC/QR pegada en la arenera: /limpie?token=<DRAWER_TOKEN> reinicia
// el contador del cajón sin PIN. Token propio (no SYNC_TOKEN) porque queda
// impreso/grabado en la etiqueta: si se filtra solo sirve para esto.
const DRAWER_TOKEN  = process.env.DRAWER_TOKEN || null;
// Skill propio de Alexa (modo desarrollo): ID "amzn1.ask.skill...." de la
// consola de Alexa. Sin él, /api/alexa queda deshabilitado.
const ALEXA_SKILL_ID = process.env.ALEXA_SKILL_ID || null;
...
async function initDB() {
  ...
  `);
  // TEMPORAL (debug conteo de visitas) — estado en vivo de la báscula para
  // el modo vigilancia (/api/watchburst). Borrar esta tabla y todo lo
  // relacionado con "watch"/vigilancia una vez confirmado que el conteo
  // de visitas está bien.
  await db.query(`
  ...
  `);
  // Preferencias de notificación: sin fila = activada
  await db.query(`
  ...
  `);
  // Preferencias generales del hogar (tema, etc.) — compartidas entre
  // dispositivos; localStorage es solo caché para pintar sin flash al cargar.
  await db.query(`
...
async function syncVisits(deviceId) {
  ...
  try {
    ...
    if (TG_TOKEN && TG_CHAT_IDS.length && nuevas.length) {
...
// ── TEMPORAL: modo vigilancia (debug conteo de visitas) ─────────────────────
// Consulta el estado en vivo de un arenero y avisa por Telegram apenas
// detecta que el peso pasó de 0 a >0 (o viceversa), con el peso crudo y el
// gato que calcula catByWeight — para comparar contra lo que ves en persona.
function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

async function checkLiveNow(devId) {
  if (!db) return;
  try {
    const st = await tuyaRequest('GET', '/v1.0/devices/' + devId + '/status');
    if (!st.success) return;
    const m = {};
    st.result.forEach(s => { m[s.code] = s.value; });
    const weight    = m.cat_weight || 0;
    const activeNow = weight > 0;

    const { rows } = await db.query('SELECT active, weight, started_at FROM live_watch WHERE device_id=$1', [devId]);
    const prev      = rows[0];
    const wasActive = prev ? prev.active : false;

    if (activeNow && !wasActive) {
      const catName = catByWeight(weight, devId);
      const kg = (weight * 0.04536).toFixed(2);
      await db.query(
        `INSERT INTO live_watch (device_id, active, weight, started_at) VALUES ($1,true,$2,$3)
         ON CONFLICT (device_id) DO UPDATE SET active=true, weight=$2, started_at=$3`,
        [devId, weight, Date.now()]
      );
      await sendTelegram('🔴 <b>AHORA</b> hay un gato en la báscula — peso: ' + weight + ' raw (' + kg +
        ' kg) → calculado: <b>' + (catName || 'Desconocido') + '</b>');
    } else if (!activeNow && wasActive) {
      const durSec = prev.started_at ? Math.round((Date.now() - Number(prev.started_at)) / 1000) : null;
      await db.query('UPDATE live_watch SET active=false WHERE device_id=$1', [devId]);
      await sendTelegram('⚪ Se bajó de la báscula' + (durSec ? ' — estuvo ' + durSec + ' s' : ''));
    }
  } catch(e) { console.error('[WATCH]', devId.slice(-6), e.message); }
}

async function getWatchUntil() {
  if (!db) return 0;
  const { rows } = await db.query("SELECT value FROM app_settings WHERE key='watch_until'");
  return rows[0] ? Number(rows[0].value) || 0 : 0;
}
async function setWatchUntil(ts) {
  if (!db) return;
  await db.query(
    `INSERT INTO app_settings (key, value) VALUES ('watch_until', $1)
     ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value`, [String(ts)]);
}
...
// "Ya vaciamos el cajón": reinicia el ciclo y rearma las alertas 80%/100%.
// La usan la app, el botón de Telegram, la etiqueta NFC/QR y Alexa.
async function resetDrawer(devId, threshold) {
  if (threshold) {
    await db.query(
  ...
  await db.query('DELETE FROM alerts_sent WHERE key = ANY($1)',
    [['drawer80:' + devId, 'drawer100:' + devId]]);
  delete _lastNotified['drawer80:' + devId];
  delete _lastNotified['drawer100:' + devId];
}

function allDeviceIds() {
  return litterboxesCache.length ? litterboxesCache.map(l => l.device_id) : (DEVICE_ID ? [DEVICE_ID] : []);
}
...
// Manda a todos los chat_id configurados (dueño + quien más lo comparta).
async function sendTelegram(text, extra) {
  if (!TG_TOKEN || !TG_CHAT_IDS.length) return false;
  let algunoOk = false;
  for (const chatId of TG_CHAT_IDS) {
    const r = await tgApi('sendMessage', Object.assign({ chat_id: chatId, text, parse_mode: 'HTML' }, extra || {}));
    if (r && r.ok) algunoOk = true;
  }
  return algunoOk;
}
...
// Preferencias de notificación (toggles en la página de configuración).
// Sin fila en BD = activada; en modo stateless viven en memoria.
const NOTIFY_KEYS = ['visit', 'drawer', 'inactivity', 'fault', 'offline', 'digest', 'trial'];
let notifyPrefsMem = {};

async function getNotifyPrefs() {
  const all = {};
  NOTIFY_KEYS.forEach(k => { all[k] = true; });
  if (!db) return Object.assign(all, notifyPrefsMem);
  try {
    const { rows } = await db.query('SELECT key, enabled FROM notify_prefs');
    rows.forEach(r => { if (Object.prototype.hasOwnProperty.call(all, r.key)) all[r.key] = r.enabled; });
  } catch(e) { console.warn('[TG] notify_prefs:', e.message); }
  return all;
}
...
async function checkAlerts() {
  ...
  try {
      ...
      try {
        ...
        for (const lb of litterboxesCache) {
          ...
          if (info.success && info.result && info.result.online === false) {
            if (prefs.offline) await notifyOnce('offline:' + lb.device_id,
              '📡 <b>' + nombre + '</b> aparece desconectada de Tuya. Revisa el enchufe o el WiFi.');
            continue;
          }
          if (!prefs.fault) continue;
    ...
    if (db && prefs.digest && horaDelDiaBogota() >= 21) {
...
<script>
// Mismo tema que el resto de la app (guardado por dispositivo)
(function() {
  try {
    var t = localStorage.getItem('gaturrin_theme');
    if (t && t !== 'pastel') document.documentElement.setAttribute('data-theme', t);
  } catch(e) {}
})();
</script>
...
<style>
:root {
  --bg:#f5eeff; --surface:#ffffff; --border:#e8d5f5;
  --pink:#d946a8; --pink-2:#e879b0; --lav:#8b5cf6;
  --text:#3b1f5e; --muted:#9d7ebe; --danger:#e11d48;
  --shadow-lg:0 8px 40px rgba(139,92,246,.15);
}
:root[data-theme="noche"] {
  --bg:#171128; --surface:#221a3a; --border:#3a2e5e;
  --pink:#f06ec2; --pink-2:#f78fd0; --lav:#a78bfa;
  --text:#f1e9ff; --muted:#a190c9; --danger:#fb7185;
  --shadow-lg:0 8px 40px rgba(0,0,0,.45);
}
:root[data-theme="menta"] {
  --bg:#eafaf2; --surface:#ffffff; --border:#cfeadd;
  --pink:#12a594; --pink-2:#3ec9b5; --lav:#059669;
  --text:#123f36; --muted:#67998a;
  --shadow-lg:0 8px 40px rgba(13,148,136,.15);
}
:root[data-theme="atardecer"] {
  --bg:#fff2e8; --surface:#fffdfa; --border:#f6dcc4;
  --pink:#ee5d3c; --pink-2:#f8935d; --lav:#a855f7;
  --text:#4a2413; --muted:#b5866a; --danger:#dc2626;
  --shadow-lg:0 8px 40px rgba(234,88,12,.16);
}
:root[data-theme="cielo"] {
  --bg:#ecf4fe; --surface:#ffffff; --border:#d4e3f8;
  --pink:#3d7ef7; --pink-2:#6ba1ff; --lav:#7c6cf0;
  --text:#1c3557; --muted:#7f9bc4;
  --shadow-lg:0 8px 40px rgba(59,130,246,.16);
}
...
input {
  ...
  background:var(--surface); font-family:'Nunito',sans-serif; font-weight:800; color:var(--text);
}
...
.err { color:var(--danger); font-size:12px; font-weight:700; margin-top:12px; min-height:16px; }
</style>
</head>
<body>
<div class="card">
...
// ── Alexa ─────────────────────────────────────────────────────────────────────
// Skill custom con endpoint HTTPS propio: Amazon firma cada request y exige
// verificarla (cadena de certificados de s3.amazonaws.com/echo.api/, SAN
// echo-api.amazon.com, firma SHA-256 sobre el body crudo, timestamp ±150 s).

const _alexaCertCache = {};

function httpsGetText(u) {
  return new Promise((resolve, reject) => {
    https.get(u, res => {
...
async function verifyAlexaRequest(req, rawBody) {
  const certUrl = String(req.headers['signaturecertchainurl'] || '');
  const sig     = String(req.headers['signature-256'] || '');
  if (!certUrl || !sig) return false;
  let u;
  try { u = new URL(certUrl); } catch { return false; }
  if (u.protocol !== 'https:' || u.hostname.toLowerCase() !== 's3.amazonaws.com' ||
      (u.port && u.port !== '443') || !u.pathname.startsWith('/echo.api/')) return false;

  let chain = _alexaCertCache[u.href];
  if (!chain) {
    const pem   = await httpsGetText(u.href);
    const certs = pem.match(/-----BEGIN CERTIFICATE-----[\s\S]+?-----END CERTIFICATE-----/g) || [];
    chain = certs.map(c => new crypto.X509Certificate(c));
    if (!chain.length) return false;
    // Cada cert firmado por el siguiente, y algún eslabón firmado por una raíz
    // de confianza (Amazon manda al final un cross-sign hacia una raíz vieja
    // que Node ya no trae, así que no basta con mirar el último).
    for (let i = 0; i < chain.length - 1; i++) {
      if (!chain[i].checkIssued(chain[i + 1]) || !chain[i].verify(chain[i + 1].publicKey)) return false;
    }
    const roots = require('tls').rootCertificates.map(r => new crypto.X509Certificate(r));
    if (!chain.some(c => roots.some(r => c.checkIssued(r) && c.verify(r.publicKey)))) return false;
    if (!/(^|,\s*)DNS:echo-api\.amazon\.com(,|$)/.test(chain[0].subjectAltName || '')) return false;
    _alexaCertCache[u.href] = chain;
  }
  const now = Date.now();
  if (now < Date.parse(chain[0].validFrom) || now > Date.parse(chain[0].validTo)) return false;
  return crypto.verify('sha256', rawBody, chain[0].publicKey, Buffer.from(sig, 'base64'));
}
...
async function handleAlexa(payload) {
  const r = payload.request || {};
  if (r.type === 'LaunchRequest') {
    return alexaSpeak('Hola. Puedes decirme: ya limpiamos, limpia la arenera, o cómo está el cajón.', false);
  }
  if (r.type !== 'IntentRequest') return alexaSpeak('');
  const intent = (r.intent && r.intent.name) || '';

  if (intent === 'YaLimpiamosIntent') {
    if (!db) return alexaSpeak('No tengo base de datos configurada, no puedo guardar la limpieza.');
    for (const id of allDeviceIds()) await resetDrawer(id);
    return alexaSpeak('Listo, reinicié el contador del cajón. ¡Gracias por limpiar!');
  }
  if (intent === 'LimpiarIntent') {
    const out = await cmd('nowclean', 'jikeclean');
    if (out && out.success) return alexaSpeak('Listo, la arenera empezó a limpiar.');
    if (out && out.code === 28841002) return alexaSpeak('No pude: venció el periodo de prueba de Tuya. Hay que renovarlo en la página de Tuya.');
    return alexaSpeak('No pude hablar con la arenera. Revisa que esté conectada.');
  }
  if (intent === 'EstadoCajonIntent') {
    const m = await getMaintenance(allDeviceIds()[0]);
    if (!m) return alexaSpeak('No tengo base de datos configurada.');
    const pct = Math.min(100, Math.round(m.visits * 100 / m.threshold));
    return alexaSpeak('El cajón va al ' + pct + ' por ciento: ' + m.visits +
      (m.visits === 1 ? ' visita' : ' visitas') + ' desde la última limpieza.');
  }
  if (intent === 'AMAZON.HelpIntent') {
    return alexaSpeak('Cuando vacíes el cajón dime: ya limpiamos. También puedo limpiar la arenera o decirte cómo va el cajón.', false);
  }
  if (intent === 'AMAZON.StopIntent' || intent === 'AMAZON.CancelIntent') return alexaSpeak('Chao.');
  return alexaSpeak('No entendí. Prueba con: ya limpiamos.', false);
}

const LIMPIE_HTML = `<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="robots" content="noindex">
<title>Cajón limpio</title>
<link href="https://fonts.googleapis.com/css2?family=Nunito:wght@600;800&display=swap" rel="stylesheet">
<style>
  body { margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center;
         background:#f5eeff; font-family:'Nunito',sans-serif; color:#4a3b5c; padding:16px; box-sizing:border-box; }
  .card { background:#fff; border-radius:24px; padding:36px 28px; text-align:center; max-width:340px;
          box-shadow:0 8px 30px rgba(217,70,168,.12); }
  .ico { font-size:64px; }
  h1 { font-size:24px; font-weight:800; margin:12px 0 8px; color:#d946a8; }
  p { margin:0; font-weight:600; opacity:.8; }
</style>
</head>
<body>
<div class="card">
  <div class="ico" id="ico">⏳</div>
  <h1 id="tit">Guardando…</h1>
  <p id="msg">Un momento</p>
</div>
<script>
// El reinicio va por POST desde JS: las vistas previas de enlaces (WhatsApp,
// el lector de QR) hacen GET sin ejecutar scripts y no deben reiniciar nada.
(function() {
  var q = location.search;
  var xhr = new XMLHttpRequest();
  xhr.open('POST', '/api/drawer/reset' + q);
  xhr.onload = function() {
    var ok = false;
    try { ok = JSON.parse(xhr.responseText).success; } catch (e) {}
    document.getElementById('ico').textContent = ok ? '🎉' : '😿';
    document.getElementById('tit').textContent = ok ? '¡Cajón limpio!' : 'No se pudo guardar';
    document.getElementById('msg').textContent = ok ? 'Contador reiniciado. ¡Gracias por limpiar!' : 'Revisa el enlace de la etiqueta o intenta desde la app.';
  };
  xhr.onerror = function() {
    document.getElementById('ico').textContent = '📡';
    document.getElementById('tit').textContent = 'Sin conexión';
    document.getElementById('msg').textContent = 'Vuelve a acercar el celular cuando tengas internet.';
  };
  xhr.send();
})();
</script>
...
const handler = async function(req, res) {
  ...
  if (pathname === '/api/telegram' && req.method === 'POST') {
    ...
    // Solo botones presionados por alguno de los chats configurados
    if (cb && cb.from && TG_CHAT_IDS.includes(String(cb.from.id))) {
      try { await ensureDB(); } catch {}
      const data = String(cb.data || '');
      let respuesta = '';
      try {
        if (data.indexOf('drawer_reset:') === 0 && db) {
          await resetDrawer(data.slice('drawer_reset:'.length));
      ...
      // TEMPORAL: modo vigilancia (debug conteo de visitas) — solo chats autorizados
      if ((text === '/vigilar' || text === '/parar') && TG_CHAT_IDS.includes(String(chatId))) {
        try { await ensureDB(); } catch {}
        if (db) {
          if (text === '/vigilar') {
            await setWatchUntil(Date.now() + 30 * 60 * 1000);
            await tgApi('sendMessage', { chat_id: chatId, text: '🔎 Modo vigilancia activado por 30 min. Te aviso apenas un gato suba a la báscula.' });
          } else {
            await setWatchUntil(0);
            await tgApi('sendMessage', { chat_id: chatId, text: '⏹️ Modo vigilancia apagado.' });
          }
        } else {
          await tgApi('sendMessage', { chat_id: chatId, text: 'Sin BD configurada, no puedo activar el modo vigilancia.' });
        }
      } else {
        // Mensaje de texto de cualquiera: le contesta su chat_id para que sea
        // fácil compartir el bot (agregarlo a TELEGRAM_CHAT_ID) sin exponer el token.
        await tgApi('sendMessage', {
          chat_id: chatId,
          text: 'Tu chat_id es: <code>' + chatId + '</code>\n\nCompártelo con quien configura Gaturrin para recibir las notificaciones.',
          parse_mode: 'HTML',
        });
      }
    }
  ...
  // Alexa: la firma de Amazon reemplaza al PIN
  if (pathname === '/api/alexa' && req.method === 'POST') {
    if (!ALEXA_SKILL_ID) { json({ success: false }, 404); return; }
    const raw = await readRawBody(req);
    let payload = {};
    try { payload = JSON.parse(raw.toString('utf8')); } catch {}
    const appId = (payload.context && payload.context.System && payload.context.System.application &&
      payload.context.System.application.applicationId) ||
      (payload.session && payload.session.application && payload.session.application.applicationId) || '';
    const ts = Date.parse((payload.request && payload.request.timestamp) || '');
    let ok = appId === ALEXA_SKILL_ID && Math.abs(Date.now() - ts) <= 150000;
    if (ok) {
      try { ok = await verifyAlexaRequest(req, raw); }
      catch (e) { console.error('[ALEXA] verificación:', e.message); ok = false; }
    }
    if (!ok) { json({ success: false }, 400); return; }
    let out;
    try { out = await handleAlexa(payload); }
    catch (e) { console.error('[ALEXA]', e.message); out = alexaSpeak('Algo falló, intenta de nuevo en un rato.'); }
    json(out);
    return;
  }

  // Etiqueta NFC/QR: la página hace el POST (ver LIMPIE_HTML)
  if (pathname === '/limpie' || pathname === '/api/drawer/reset') {
    const tokOk = DRAWER_TOKEN && safeEqual(String(parsed.query.token || ''), DRAWER_TOKEN);
    if (!tokOk) { json({ success: false, msg: 'Token inválido' }, 401); return; }
    if (pathname === '/limpie') {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
      res.end(LIMPIE_HTML);
      return;
    }
    if (req.method !== 'POST') { json({ success: false }, 405); return; }
    if (!db) { json({ success: false, msg: 'Sin BD' }); return; }
    const ids = parsed.query.device ? allDeviceIds().filter(id => id === parsed.query.device) : allDeviceIds();
    if (!ids.length) { json({ success: false, msg: 'Arenero desconocido' }, 404); return; }
    for (const id of ids) await resetDrawer(id);
    json({ success: true });
    return;
  }

  // Cron externo: /api/sync, /api/clean y /api/watchburst con ?token=<SYNC_TOKEN>
  // pasan sin cookie (sync periódico, limpiezas y vigilancia desde pg_cron)
  const cronOk = SYNC_TOKEN && (pathname === '/api/sync' || pathname === '/api/clean' || pathname === '/api/watchburst') &&
  ...
  if (pathname.startsWith('/api')) {
    ...
    try {
      ...
      // ── Ajustes generales (tema, etc.) — compartidos entre dispositivos ──────
      const THEME_IDS = ['pastel', 'noche', 'menta', 'atardecer', 'cielo'];
      if (pathname === '/api/settings' && req.method === 'GET') {
        let theme = 'pastel';
        if (db) {
          try {
            const { rows } = await db.query("SELECT value FROM app_settings WHERE key='theme'");
            if (rows.length) theme = rows[0].value;
          } catch(e) { console.warn('[settings] get:', e.message); }
        }
        json({ success: true, result: { theme } });
        return;

      } else if (pathname === '/api/settings/save' && req.method === 'POST') {
        const p = JSON.parse(body || '{}');
        if (p.key === 'theme') {
          if (THEME_IDS.indexOf(p.value) < 0) { json({ success: false, msg: 'Tema inválido' }, 400); return; }
          if (db) {
            await db.query(
        ...
        json({ success: false, msg: 'Clave inválida' }, 400);
        return;
      }

      // ── Preferencias de notificación ──────────────────────────────────────────
      if (pathname === '/api/notifyprefs' && req.method === 'GET') {
        json({ success: true, result: await getNotifyPrefs(), persisted: !!db });
        return;

      } else if (pathname === '/api/notifyprefs/save' && req.method === 'POST') {
        const p = JSON.parse(body || '{}');
        const key = String(p.key || '');
        if (NOTIFY_KEYS.indexOf(key) < 0) { json({ success: false, msg: 'Clave inválida' }, 400); return; }
        const enabled = !!p.enabled;
        if (db) {
          await db.query(
            'INSERT INTO notify_prefs (key, enabled) VALUES ($1,$2) ON CONFLICT (key) DO UPDATE SET enabled=EXCLUDED.enabled',
            [key, enabled]);
        } else {
          notifyPrefsMem[key] = enabled;
        }
        json({ success: true, persisted: !!db });
        return;
      }
      ...
      if (pathname === '/api/maintenance') {
        ...
        await resetDrawer(devIdM, th);
        json({ success: true });
        return;

      } else if (pathname === '/api/visits/reassign' && req.method === 'POST') {
        // Corrección manual: "esta visita fue de otro gato" (null = Desconocido)
        if (!db) { json({ success: false, msg: 'Sin BD' }); return; }
        const p = JSON.parse(body || '{}');
        const tsR = Number(p.ts);
        if (!tsR) { json({ success: false, msg: 'ts requerido' }, 400); return; }
        const devIdR = p.device_id || (litterboxesCache[0] && litterboxesCache[0].device_id) || DEVICE_ID;
        const catR = (p.cat_name === null || p.cat_name === undefined || p.cat_name === '') ? null : String(p.cat_name);
        if (catR && !catsCache.some(c => c.name === catR)) { json({ success: false, msg: 'Ese gato no existe' }, 400); return; }
        const { rowCount } = await db.query(
          'UPDATE visits SET cat_name=$1 WHERE device_id=$2 AND ts=$3', [catR, devIdR, tsR]);
        if (!rowCount) { json({ success: false, msg: 'Visita no encontrada' }, 404); return; }
        json({ success: true });
        return;

      } else if (pathname === '/api/visits/delete' && req.method === 'POST') {
        // Borra un registro (p.ej. un "desconocido" espurio). Nota: si es la
        // visita más reciente del dispositivo, el próximo sync puede volver a
        // traerla desde Tuya (retiene logs ~7 días) porque el watermark de
        // sync usa MAX(ts) de la tabla.
        if (!db) { json({ success: false, msg: 'Sin BD' }); return; }
        const pD = JSON.parse(body || '{}');
        const tsDel = Number(pD.ts);
        if (!tsDel) { json({ success: false, msg: 'ts requerido' }, 400); return; }
        const devIdDel = pD.device_id || (litterboxesCache[0] && litterboxesCache[0].device_id) || DEVICE_ID;
        const { rowCount: delCount } = await db.query(
          'DELETE FROM visits WHERE device_id=$1 AND ts=$2', [devIdDel, tsDel]);
        if (!delCount) { json({ success: false, msg: 'Visita no encontrada' }, 404); return; }
        json({ success: true });
        return;

      } else if (pathname === '/api/maintenance/threshold' && req.method === 'POST') {
        // Ajusta solo la capacidad del cajón; NO reinicia el ciclo en curso
        if (!db) { json({ success: false, msg: 'Sin BD' }); return; }
        const p = JSON.parse(body || '{}');
        const devIdM = p.device_id || parsed.query.device || (litterboxesCache[0] && litterboxesCache[0].device_id) || DEVICE_ID;
        const th = Math.round(Number(p.threshold));
        if (!(th >= 10 && th <= 500)) { json({ success: false, msg: 'Debe estar entre 10 y 500' }, 400); return; }
        await db.query(
          'INSERT INTO maintenance (device_id, last_emptied, threshold) VALUES ($1,$2,$3) ON CONFLICT (device_id) DO UPDATE SET threshold=EXCLUDED.threshold',
          [devIdM, Date.now(), th]);
        json({ success: true });
        return;
      ...
      // TEMPORAL (debug conteo de visitas): mientras el modo vigilancia esté
      // activo (ver /api/telegram → /vigilar), sondea cada ~10s durante esta
      // invocación y avisa por Telegram al segundo. pg_cron llama esta ruta
      // cada minuto; si no está activo, no gasta nada (solo un SELECT).
      } else if (pathname === '/api/watchburst') {
        const until = await getWatchUntil();
        if (!until || Date.now() > until) { json({ success: true, active: false }); return; }
        const devices = litterboxesCache.length ? litterboxesCache : (DEVICE_ID ? [{ device_id: DEVICE_ID }] : []);
        const deadline = Date.now() + 50000; // margen bajo el timeout de la función
        while (Date.now() < deadline) {
          const stillOn = await getWatchUntil();
          if (!stillOn || Date.now() > stillOn) break;
          for (const lb of devices) await checkLiveNow(lb.device_id);
          if (Date.now() + 10000 < deadline) await sleep(10000); else break;
        }
        ...
        if (db) {
          ...
          // cat: nombre guardado al sincronizar (o reasignado a mano) — es la
          // fuente de verdad; el frontend solo re-identifica como fallback
          const { rows } = await db.query(
            `SELECT ts, cat_name, weight_raw AS weight, duration_sec AS duration
             FROM visits WHERE device_id=$1 ORDER BY ts DESC LIMIT 2000`, [reqDevId]
          );
          json({ success: true, result: rows.map(r => ({
            ts: Number(r.ts), cat: r.cat_name, weight: Number(r.weight), duration: r.duration ? Number(r.duration) : null
          })), source: 'db' });
        } else {
          ...
          json({ success: true, result: parseVisits(logs).map(v =>
            Object.assign({ cat: catByWeight(v.weight, reqDevId) }, v)), source: 'tuya' });
        }
