...
const handler = async function(req, res) {
  ...
  if (pathname.startsWith('/api')) {
    ...
    try {
      ...
      // TEMP: diagnóstico del webhook de Telegram (quitar cuando se resuelva).
      if (pathname === '/api/telegram/debug' && req.method === 'GET') {
        const qsDbg = new URL('http://x' + req.url).searchParams;
        let fixResult = null;
        if (qsDbg.get('fix') === '1') {
          fixResult = await tgApi('setWebhook', {
            url: APP_URL + '/api/telegram',
            secret_token: SYNC_TOKEN,
            allowed_updates: ['callback_query', 'message'],
          });
        }
        const info = await tgApi('getWebhookInfo', {});
        json({ success: true, chat_ids_configurados: TG_CHAT_IDS, webhook_info: info, fix_result: fixResult });
        return;
      }
